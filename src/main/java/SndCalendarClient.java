 ...
 */
public class SndCalendarClient {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            Event event1 = new Event("Meeting-2", LocalDateTime.of(2023, 12, 5, 5, 30), 60, "Business", "Discuss project", "Office");
            calendar.addEvent(event1);

            // Sample Test 2: Try to add an event with the same name (should print an error message)
            Event duplicateEvent = new Event("Meeting-2", LocalDateTime.of(2023, 12, 6, 8, 30), 90, "Business", "Another discussion", "Coffee Shop");
            calendar.addEvent(duplicateEvent);

            // Sample Test 3: Add another event
            Event event2 = new Event("Conference-2", LocalDateTime.now().plusDays(1), 120, "Tech", "Annual tech conference", "Conference Hall");
            calendar.addEvent(event2);

            // Sample Test 4: Get events by name
            List<Event> eventsByName = calendar.getEventsByName("Meeting-2");
